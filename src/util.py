...
if sys.version_info[0] > 2:
    from collections import MutableMapping as DM

else:
    import UserDict.DictMixin as DM
#-- Message writers --{{{1
...
class Message (object):
    ...
    def format_pos (self, where, text):
        ...
        """
        if len(self.pos) > 0:
            if where is None or not 'file' in where:
                where = self.pos[-1]
...
class Variables(DM):
    ...
    def __init__ (self, parent = None, items = {}):
        ...
        #    raise ValueError('parent:{} type:{}'.format(
        #            parent, type(parent)))
        self.parent = parent
        self.dict = items
    ...
    def __getitem__ (self, key):
        ...
        while object is not None:
            if key in object.dict: #.has_key(key):
